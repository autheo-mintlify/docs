...
**Fix**: The address may have no on-chain activity, or the transaction hash may be incorrect. Verify on the [block explorer](https://testnet-explorer.autheo.com/).
...
async function apiRequest(path) {
  const response = await fetch(`https://testnet-explorer.autheo.com/api/v2${path}`);
