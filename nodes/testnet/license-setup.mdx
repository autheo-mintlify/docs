...
Purchase the appropriate ERC-721 NFT from an authorized distributor on Arbitrum. View contract addresses:
...
Send the NFT to your EVM address (`0x...` format). This triggers an automatic on-chain hook that mints a `LicenseRecord` in `ISSUED` status — no separate transaction needed.
