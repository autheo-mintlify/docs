...
Autheo Chain (`autheo_785-1`) is a permissioned, EVM-compatible Layer 1 blockchain built on Cosmos SDK v0.53.0 and Ethermint. It features a custom NFT license gating system requiring validators and delegators to hold on-chain license NFTs.
...
Always pass `--chain-id autheo_785-1` to CLI commands.
...
Yes — full Ethermint EVM with JSON-RPC on port 8545 (HTTP) and 8546 (WebSocket). MetaMask, ethers.js, Hardhat, and Foundry all work with Chain ID `785`.
...
[https://github.com/autheo-blockchain/autheo-chain-lab](https://github.com/autheo-blockchain/autheo-chain-lab)
...
autheod tx staking unbond <src-valoper> <amount>aauth \
  --from <key> --chain-id autheo_785-1 --keyring-backend file
...
autheod tx licensedistribution claim-nft-rewards <license-id> \
  --from <owner-key> --chain-id autheo_785-1 --keyring-backend file
```
...
autheod tx bank send <sender-key> <recipient-address> <amount>aauth \
  --chain-id autheo_785-1 --keyring-backend file \
