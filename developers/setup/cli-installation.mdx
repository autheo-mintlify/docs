...
You can get the latest `autheod` binary here from the [testnet page](https://github.com/autheo-platform/testnet/tree/autheo-testnet-dev).
...
```
$ autheod config "chain-id" autheo_785-1
$ autheod config
{
...
After the necessary changes are made in the `client.toml`, then save. For example, if we directly change the `chain-id` to `autheo_785-1` and output to number, it would change instantly as shown below.
