# Autheo Chain: Self-Hosting Guide

This document covers running an Autheo node from scratch. Two deployment methods are covered: **systemd** (binary) and **Docker**.

---

## Table of Contents

1. [Chain Reference](#1-chain-reference)
2. [Method A: systemd](#2-method-a-systemd)
   - [Prerequisites](#21-prerequisites)
   - [Build the Binary](#22-build-the-binary)
   - [Join the Network](#23-join-the-network)
   - [Configure the Node](#24-configure-the-node)
   - [Start with systemd](#25-start-with-systemd)
3. [Method B: Docker](#3-method-b-docker)
   - [Pull the Image](#31-pull-the-image)
   - [Prepare the Data Directory](#32-prepare-the-data-directory)
   - [Join the Network](#33-join-the-network-docker)
   - [Configure the Node](#34-configure-the-node-docker)
   - [Run the Container](#35-run-the-container)
4. [Sync Methods](#4-sync-methods)
   - [Option A: State Sync (recommended)](#option-a-state-sync-recommended)
   - [Option B: Genesis Sync](#option-b-genesis-sync)
5. [Verify the Node](#5-verify-the-node)
6. [Key Management](#6-key-management)
7. [Create a Validator](#7-create-a-validator)
...
# Go 1.25 (must match go.mod - verify exact patch version at https://go.dev/dl/)
GO_VERSION=1.25.0
...
Then choose a sync method from [section 4](#4-sync-methods).
...
```bash
docker pull ghcr.io/autheo-blockchain/autheod:1.0.16
```

### 3.2 Prepare the Data Directory
...
docker run --rm \
  ...
  ghcr.io/autheo-blockchain/autheod:1.0.16 \
...
Then apply the sync method of your choice from [section 4](#4-sync-methods) using the host-side config files.

### 3.4 Configure the Node (Docker)

Apply config changes directly to the host-side directory. Use the same `sed` commands from [section 2.4](#24-configure-the-node) - they operate on `$HOME/.autheo/config/` which is mounted into the container.
...
> **Trade-off:** No historical data before the snapshot height.

### Option B: Genesis Sync

Replays every block from block 1. Required only for a full archive node.

> **Note:** The chain has had no governance upgrades since genesis. No additional setup is needed.

If the chain has governance upgrades in the future, genesis sync will require **cosmovisor** to manage binary swaps at each upgrade height. Historical upgrade plan names, heights and binaries will be listed in `mainnet/upgrades.json` in the networks repo. The file follows this structure:
...
Keyring backends:
- `file` - encrypted file on disk, passphrase required on every use (recommended for production)
- `os` - OS keychain (macOS Keychain, Linux Secret Service)
- `test` - plaintext on disk, no passphrase (development only)
...
**Step 1 - Verify your Sovereign license is BOUND to this validator:**
...
```bash
# systemd
# For a Sovereign license, --validator-address defaults to the key's own validator address
